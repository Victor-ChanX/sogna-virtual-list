...
/** @internal **/
export type SR<
  E extends AnySystemSpec,
  R extends System = ReturnType<E["constructor"]>,
> = R;
 ...
 */
export type SystemConstructor<D extends SystemSpecs> = (
  dependencies: SpecResults<D>,
) => System;
...
export interface SystemSpec<
  SS extends SystemSpecs,
  C extends SystemConstructor<SS>,
> {
  constructor: C;
  dependencies: SS;
  id: string;
...
export function init<SS extends AnySystemSpec>(systemSpec: SS): SR<SS> {
  ...
  const _init = <SS extends AnySystemSpec>({
    constructor,
    dependencies,
    id,
    singleton,
    ...
    const system = constructor(
      dependencies.map((e: AnySystemSpec) => _init(e)),
    );
    if (singleton) {
